...
TreePtr LeafNode::insert_key(const Key &key, const RecordPtr &record_ptr) {
    ...
    if (this->size < FANOUT) {
        ...
        temp->load();
        new_leaf = temp->tree_ptr;
        temp->next_leaf_ptr = this->next_leaf_ptr;
        this->next_leaf_ptr = new_leaf;
        this->data_pointers[key] = record_ptr;
        this->size++;
        while (this->size > MIN_OCCUPANCY) {
            auto it = this->data_pointers.rbegin();
            temp->data_pointers[it->first] = it->second;
...
void LeafNode::delete_key(const Key &key) {
    // cout << "LeafNode::delete_key" << endl;

    this->load();
    
    size_t count = this->data_pointers.erase(key);
    this->size -= count;
